...
const DashboardLayout = ({ children }) => {
  ...
  const menuItems = [
    { name: "Recent Summaries", path: "/dashboard/summary" },
              ...
              <ul>
                {menuItems.map((item, index) => (
                  <li key={index}>
                    <Link href={item.path}>
                      <div
                        className={`pl-5 pr-2 py-4 hover:bg-blue-900 cursor-pointer text-md font-thin flex items-center rounded-full my-2 mx-2 ${
                          item.name === "Saved Summaries" ? "bg-blue-900" : ""
                        }`}
                      >
                        <span>{item.name}</span>
                      </div>
