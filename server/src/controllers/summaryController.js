...
const createSummary = async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      return res
        .status(400)
        .json({ message: "No file uploaded or text provided" });
    }
    ...
    res
      .status(500)
      .json({ error: "An error occurred while creating the summary" });
  } finally {
...
const getSummaries = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while fetching summaries" });
  }
};
...
const deleteSummary = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while deleting the summary" });
  }
};
